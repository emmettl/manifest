...
| `src/maritime/fleet-geometry.ts` | Resident original-sample edges, dateline splitting, current heads and demand-driven picking |
| `src/maritime/fleet-webgl.ts` | Instanced GPU wakes and points, six draw calls and per-chunk geometry uploads |
| `src/maritime/fleet-canvas.ts` | Batched fallback for unavailable/lost WebGL and known software rasterizers |
...
- The on-map meter reports canvas marks drawn, CPU preparation/submission duration (p95 over roughly one second during playback), and completed draws per second. Paused redraws report one draw duration and exclude idle time. The 16.7 ms target covers the whole frame, so a draw below budget alone does not establish 60 fps. React, browser painting, GPU completion and data loading are outside the draw timer. Counts with positions respect the time and category filters; viewport culling changes drawn marks, not the loaded fleet.
...
- Spatial subdivision, physical-device GPU measurement, adaptive level of detail, port-call inference, provider adapters, static identity histories and trade statistics are next-stage work.
...
The default hardware path uploads original sample-pair edges once per chunk. The GPU clips them by time, fades their full 1.4-day wake (three days when selected), and draws every qualifying head. Local review retains its 30-minute/one-hour wake durations. Separate logical segments never share an edge; explicit seam splits prevent dateline-spanning chords. Exact CPU head interpolation controls segment activity, class visibility, viewport counts and picking. The shader follows the original piecewise-linear route rather than the former 24 resampled wake strokes. Three wrapped copies use six draw calls in total.

Canvas land redraws only when the camera, viewport or rendering backend changes. GPU buffers are reused during playback and replaced on daily transitions. A reusable texture holds current heads; it does not carry the complete archive. Known software GL backends and unavailable/lost WebGL use a batched Canvas fallback with the same original edges and wake lengths, quantizing fades into 16 buckets. Restoration recreates resources from the currently displayed day. The accessible map canvas owns gestures; the transparent fleet layer is decorative and does not intercept pointer input. Selection labels remain an SVG overlay.

The animation callback commits the React clock update synchronously to avoid an extra frame of scheduling delay. Expensive whole-fleet dropdown construction remains bounded to 100 choices. Submission duration excludes GPU completion; the benchmark also records observed frame intervals and the actual backend. See `PERFORMANCE.md` for measured improvements and remaining limits.
