...
Every visible port has an SVG marker and accessible name. Text labels avoid other labels, markers and main controls. Selected ports and demo voyage endpoints get priority; other names appear where space permits. The searchable Ports directory lists every port, including those outside the viewport, and centers/zooms the map on selection. Aliases cover aggregate and alternate names. Layout is memoized across playback frames. Passages remain separate from ports.
