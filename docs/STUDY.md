...
Source audit extended on 8 September 2026 with the free regional archives and derived port datasets listed below; the verdict is unchanged.
...
| Public regional individual tracks | **Green, but not global** | NOAA publishes daily AIS downloads for US waters; the Danish authorities publish daily raw AIS for Danish waters from 2006 to the present; Norway publishes open live and historical AIS under NLOD; Australia publishes hourly-thinned monthly traffic for its search-and-rescue region. These prove the compiler and visual grammar and, for Denmark and Australia, touch real export lanes (Baltic tanker traffic, Pilbara bulk), but none supports China–Europe or Gulf–Asia/Europe continuity. |
...
| [Spire Maritime](https://spire.com/maritime/) | Combined satellite, terrestrial and enhanced AIS; live APIs/feeds, vessel/port enrichment, and historical data back to 2011. The historical API supports track/point delivery and bulk requests. | Commercial, contract and quote based. Public documentation establishes technical capability, not permission to redistribute browser-ready derived tracks. | **No longer an independent lead.** Spire's maritime business was acquired by Kpler in 2025, so Spire, MarineTraffic and Kpler AIS are one counterparty. Obtain at least one independent bulk quote — ORBCOMM, Lloyd's List Intelligence, Windward or Pole Star — before negotiating publication rights, if only to anchor price and terms. |
...
| [Danish AIS archive](http://aisdata.ais.dk/) ([DMA AIS data page](https://www.dma.dk/safety-at-sea/navigational-information/ais-data)) | Terrestrial AIS for Danish waters as zipped daily CSV from March 2006 (monthly files in the earliest years) to the present; on 8 September 2026 the newest file was `aisdk-2026-09-04.zip`, and the 1–5 January 2025 files are 480–555 MB each. Twenty-six columns include class, MMSI, IMO, name, call sign, ship type, cargo type, dimensions, draught, destination and ETA — richer static data than the NOAA release. DMA states that “Historical AIS-data are free for down-load” under the Danish public-sector-information act (act no. 596 of 24 June 2005). The [AIS data management policy](https://www.dma.dk/safety-at-sea/navigational-information/ais-data/ais-data-management-policy-) governs paid live access; it forbids combining data so that persons become identifiable without a Danish Data Protection Agency permit and disclaims correctness. No licence text names publication or redistribution. The land-based system is now run by the Danish Emergency Management Agency. | The deepest free raw archive outside the US and the only one crossing a defining trade lane: the Skagerrak/Kattegat/Great Belt entrance carries Baltic tanker exports and feeder container traffic. Same 1–3 January 2025 window as the NOAA sample. Publication terms need the same written clarification as NOAA; the [prepared request](AIS-DATA-REQUESTS.md) covers it. |
| [Norwegian Coastal Administration](https://www.kystverket.no/en/sea-transport-and-ports/ais/access-to-ais-data/) | Free open real-time and [historical AIS](https://hais.kystverket.no/about) for Norwegian waters, the EEZ and the Svalbard/Jan Mayen protection zones, satellite and terrestrial, with historical extracts delivered as Parquet. Licensed under the Norwegian Licence for Open Government Data (NLOD), which permits redistribution and derived works with Kystverket credit. Open data excludes fishing vessels under 15 m and leisure vessels under 45 m. | The only raw AIS source with an explicit permissive licence; use it when a fixture must be free of publication ambiguity. Geographically peripheral to the defining flows. |
| [Spatial@AMSA vessel traffic data](https://www.operations.amsa.gov.au/spatial/DataServices/DigitalData) | Monthly datasets for Australia's whole search-and-rescue region, January 2020 to August 2026, from the Craft Tracking System (terrestrial and satellite AIS), thinned to one position per vessel per 60 minutes, plus denser sub-area sets for Bass Strait, the Great Australian Bight and the North West. Derivative listings state Creative Commons Attribution-NonCommercial 3.0 Australia; the AMSA page itself states no licence, so confirm the notice inside the download package. | Hourly thinning makes this presence rather than continuous track evidence, but it covers the Pilbara iron-ore and North West LNG lanes toward China, pairing with the port statistics already collected. Non-commercial only. |
| [Fintraffic Digitraffic marine AIS](https://www.digitraffic.fi/en/marine-traffic/) | Live vessel positions and metadata for the Baltic near Finland over MQTT/WebSocket under Creative Commons Attribution 4.0; fishing vessels are filtered out. No historical archive was found. | Cleanly licensed live regional spike only. Peripheral. |
...
The NOAA and Danish releases share the same gap: an explicit access grant with no licence text naming publication or redistribution. The starting positions differ. Works of the United States government are not subject to US copyright, and vessel positions are facts; the InPort “use constraint” of “For coastal and ocean planning” is descriptive metadata about the product's purpose, not a licence term. The prepared NOAA request therefore documents intended use and attribution rather than seeking a grant that the law does not require; the remaining NOAA-side constraints are the accuracy and liability disclaimer and the IMO security caution, which is editorial. European releases sit under the EU sui generis database right and, for Denmark, a data-protection condition, so an express licence or written confirmation matters more there; NLOD and CC BY resolve it, DMA's “free for down-load” does not yet. None of this is legal advice; record the reasoning in the source manifest and have it checked before the first public observed fixture.
...
- [IMF PortWatch](https://portwatch.imf.org/pages/data-and-methodology) publishes daily port-call counts and estimated import/export tonnage for about 2,065 ports from 2019, derived from UN Global Platform satellite AIS and updated weekly, downloadable as CSV or through an ArcGIS Hub API. The catalogue's licence field points at the IMF's general terms, which were not readable by automated fetch and need a manual check before use. It can supply the roadmap's source-bounded port-call and trade-context layers, but as *estimates*: keep it visibly separate from the reported port-authority figures in `DATA-SOURCES.json`.
- [Eurostat maritime transport](https://ec.europa.eu/eurostat/cache/metadata/en/mar_esms.htm) (`mar_go` tables) reports gross weight and TEU handled at EU main ports quarterly and annually under Eurostat's standard reuse policy. One dataset could replace the individually transcribed European port-authority reports and give consistent periods and scope.
...
Before ingesting a global archive, obtain the same evaluation slice from Kpler (which now includes Spire and MarineTraffic) and at least one independent provider: 72 hours around the Yangtze/Pearl River approaches, Malacca, Hormuz and Suez/Bab el-Mandeb, plus a quote for one global 30-day historical window. The slice must include dynamic positions, message/receive time, source type, MMSI and time-aligned static/voyage messages; quote port calls and characteristics separately so their value is measurable.
...
Until then it is a strong global-motion investigation, not a promised edition. The immediate proof is deliberately small: compile the NOAA Los Angeles/Long Beach sample (done) and the Danish Baltic-entrance sample for the same 1–3 January 2025 window through one compiler, prototype the global density field through Global Fishing Watch, and compare identical commercial evaluation slices before choosing whether individual world tracks are worth their rights and cost.
