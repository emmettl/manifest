...
describe('native canvas gesture listeners', () => {
  ...
  it.each([0, 1, 2])('zooms ordinary wheel input around the cursor in delta mode %i', deltaMode => {
    const view = setup()
    const anchor = { clientX: 710, clientY: 320 }
    const before = geoAt(initial, 700, 300)
    const scroll = view.send('wheel', { ...anchor, ctrlKey: false, deltaY: -1, deltaMode })
    expect(scroll.defaultPrevented).toBe(true)
    expect(view.camera().zoom).toBeGreaterThan(initial.zoom)
    geoAt(view.camera(), 700, 300).forEach((value, i) => expect(value).toBeCloseTo(before[i]))
    view.send('wheel', { ...anchor, ctrlKey: false, deltaY: 1, deltaMode })
    expect(view.camera().zoom).toBeCloseTo(initial.zoom)
    expect(view.taps()).toBe(0)
    view.cleanup()
    const previous = view.camera()
    const detached = view.send('wheel', { ctrlKey: false, deltaY: -50, deltaMode: 0 })
    expect(detached.defaultPrevented).toBe(false)
    expect(view.camera()).toEqual(previous)
  })
  it('handles trackpad pinch and removes its listener on cleanup', () => {
    const view = setup()
