...
export function parseStudy(value: unknown): TrackStudy {
  ...
  // This public prototype admits only the authored synthetic fixture.
  if (value.source.evidence !== 'synthetic' || value.source.publication !== 'synthetic-only' || typeof value.source.license !== 'string') throw new Error('This prototype requires an explicitly synthetic fixture.')
  const ids = new Set<string>()
  for (const vessel of value.vessels) {
    if (!object(vessel) || typeof vessel.id !== 'string' || ids.has(vessel.id) || typeof vessel.label !== 'string' || !['cargo', 'tanker', 'other'].includes(String(vessel.category)) || vessel.evidence !== 'synthetic') throw new Error('Invalid vessel record.')
    ids.add(vessel.id)
  }
